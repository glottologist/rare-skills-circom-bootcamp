...
#include "circom.hpp"
#include "calcwit.hpp"
void AllBinary_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void AllBinary_0_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "AllBinary";
...
Fr_sub(&expaux[2],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 0)],&circuitConstants[2]); // line circom 12
Fr_mul(&expaux[1],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 0)],&expaux[2]); // line circom 12
...
AllBinary_0_create(1,0,ctx,"main",0);
AllBinary_0_run(0,ctx);
}
