...
#include "circom.hpp"
#include "calcwit.hpp"
void MultiANDNoOut_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MultiANDNoOut_0_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "MultiANDNoOut";
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 0)],&circuitConstants[2]); // line circom 12
...
MultiANDNoOut_0_create(1,0,ctx,"main",0);
MultiANDNoOut_0_run(0,ctx);
}
